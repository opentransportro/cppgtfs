...
namespace csv
{
    ...
    bool CsvParser::readNextLine()
    {
        ...
        while (pos < _currentLine.size()) {
            if (!firstChar && (std::isspace(_currentLine[pos]) != 0)) {
                pos++;
                lastPos = pos;
                continue;
            }
            ...
            if (esc_quotes_found == 0) {
                ...
                _currentModItems.emplace_back(_currentLine.c_str() + lastPos);

                while (esc_quotes_found != 0) {
    ...
    bool CsvParser::lineIsEmpty(const char* line) const
    {
        size_t i = 0;
        while (line[i] != 0) {
            if (isspace(line[i]) == 0) { return false; }
            i++;
        }
    ...
    bool CsvParser::isDouble(std::string line, bool notEmpty)
    {
        strtrim(&line);
    ...
    // ___________________________________________________________________________
    void CsvParser::ltrim(std::string* s)
    {
        s->erase(s->begin(), std::find_if(s->begin(), s->end(), std::not1(std::ptr_fun<int, int>(std::isspace))));
    }

    // ___________________________________________________________________________
    void CsvParser::rtrim(std::string* s)
    {
        s->erase(std::find_if(s->rbegin(), s->rend(), std::not1(std::ptr_fun<int, int>(std::isspace))).base(), s->end());
    }

    // ___________________________________________________________________________
    void CsvParser::strtrim(std::string* s)
    {
        ltrim(s);
        rtrim(s);
    }
    ...
    // ___________________________________________________________________________
    bool CsvParser::isLong(std::string line)
    {
        strtrim(&line);
        char* p{ nullptr };
    ...
    // ___________________________________________________________________________
    bool CsvParser::isLong(std::string line, bool notEmpty)
    {
        strtrim(&line);
        if (line.empty() && notEmpty) {
            return false;
    ...
    // ___________________________________________________________________________
    bool CsvParser::isDouble(std::string line)
    {
        strtrim(&line);
        char* p{ nullptr };
        strtod(line.c_str(), &p);
