...
// ___________________________________________________________________________
    ...
    gtfs::flat::ServiceDate DataExtractor::getServiceDate(const CsvParser& csv, size_t field) const
    {
...
// ____________________________________________________________________________
    gtfs::flat::ServiceDate DataExtractor::getServiceDate(const CsvParser& csv, size_t field, bool req) const
    {
        size_t p;
        const char* val = csv.getTString(field);
        if (strlen(val) == 0 && !req) return gtfs::flat::ServiceDate();
        ...
        try {
            ...
            return gtfs::flat::ServiceDate(yyyymmdd);
    ...
    // ____________________________________________________________________________
    gtfs::flat::Time DataExtractor::getTime(const CsvParser& csv, size_t field) const
    {
        size_t p;
        const char* val = csv.getTString(field);

        if (val == nullptr || val[0] == 0) {
            return gtfs::flat::Time();
        }
        ...
        try {
            ...
            return gtfs::flat::Time(h, m % 60, s % 60);
        } catch (const std::exception& e) {
            std::stringstream msg;
    ...
    // ____________________________________________________________________________
    gtfs::flat::Route::TYPE DataExtractor::getRouteType(const CsvParser& csv, size_t field, int64_t tn) const
    {
